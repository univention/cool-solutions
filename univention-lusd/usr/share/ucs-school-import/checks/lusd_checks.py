...
class LUSDImportGPGConfigurationChecks(ConfigurationChecks):
	...
	def test_03_gpg_passphrase_file(self):
		if not os.path.exists(self.config['passphrase_file']):
			raise InitialisationError('Passphrase file {!r} does not exist.'.format(self.config['passphrase_file']))
		try:
			with open(self.config['passphrase_file'], 'r') as fp:
				passphrase = fp.read().strip()
		except IOError as exc:
			raise InitialisationError('Reading passphrase file {!r}: {}'.format(self.config['passphrase_file'], exc))
		if not passphrase:
			raise InitialisationError('Passphrase file {!r} is empty.'.format(self.config['passphrase_file']))

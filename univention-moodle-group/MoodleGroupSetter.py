...
class MoodleGroupSetter(ListenerModuleHandler):
    ...
    def create(self, dn, new):
        ...
        if not "univentionFreeAttributes".encode("utf-8") in new["objectClass"]:
            changes.append(
                ("objectClass", "", "univentionFreeAttributes".encode("utf-8"))
            )
        if "memberUid" in new:
            for i in new["memberUid"]:
                ...
                try:
                    result = self.lo.search(
                        filter="(&(uid=%s)(ucsschoolRole=student*))"
                        % i.decode("utf-8"),
                        attr=["dn"],
                    )
                    if len(result) != 0:
                    ...
                    result = self.lo.search(
                        filter="(&(uid=%s)(ucsschoolRole=teacher*))"
                        % i.decode("utf-8"),
                        attr=["dn"],
                    )
                    if len(result) != 0:
                    ...
                    self.logger.error(
                        "Ldap Search error for user: %s" % i.decode("utf-8")
                    )
    ...
    def modify(self, dn, old, new, old_dn):
        ...
        if "memberUid" in currentdiff:
            ...
            if memberdiff[0] is None:
                removed = set()
                added = set(memberdiff[1])
            elif memberdiff[1] is None:
            ...
            if len(removed) != 0:
                for i in removed:
                    if (
                        "univentionFreeAttribute19" in old
                        and i in old["univentionFreeAttribute19"]
                    ):
                        changes.append(("univentionFreeAttribute19", i, ""))
                    if (
                        "univentionFreeAttribute20" in old
                        and i in old["univentionFreeAttribute20"]
                    ):
                        changes.append(("univentionFreeAttribute20", i, ""))

...
def applyOUMapping(dn):
    for ou in ouMappingList:
        if re.match(r'ou\={}'.format(ou), dn):
            dn = re.sub(r'ou\={}'.format(ou), r'ou\={}'.format(UCRV['dataport/user_group_sync/sink/mapping/{}'.format(ou)]), dn)
    #if not newDN:
    return dn
...
def readUCRV(usedUCRVdefaults):
    ...
    for (variable, defaultValue, ) in usedUCRVdefaults.items():
        if type(defaultValue) is bool:
            variables[variable] = ucr.is_true(variable, defaultValue)
        elif type(defaultValue) is int:
            variables[variable] = int(ucr.get(variable, defaultValue))
        else:
            variables[variable] = ucr.get(variable, defaultValue)
...
def _connectToLdap():
    ...
    global  udmUserSearch, udmModuleUsersUser
    ...
    #positionUser = re.sub(r'(dc\=.*$)', UCRV['ldap/base'], positionUser)

    #TODO DN hier nicht hart setzen, sondern in import_user nutzen

#    positionGroup = re.sub(r'(dc\=.*$)', UCRV['ldap/base'], positionGroup)
...
def udmUserNews(position):
    global udmUserNew
#    lo, position = univention.admin.uldap.getAdminConnection()
    #position = re.sub(r'(dc\=.*$)', UCRV['ldap/base'], user_dn)
    #position = re.sub(r'uid=[a-zA-Z0-9-_]*,', '', position)
    #with open('/var/log/val-debug.log', 'a') as f:
    #    f.write(position)
    #position = univention.admin.uldap.position(applyOUMapping(position))
    #return udmModuleUsersUser.object(configObject, ldapObject, position)
    udmUserNew = lambda: udmModuleUsersUser  .object(configObject, ldapObject, position)

#    return univention.admin.modules.get('users/user').object(None, lo, position)
...
def getPosition(user_dn):
    ...
    #    f.write(position)
    return position
    #return univention.admin.uldap.position(applyOUMapping(position))
...
def _user_should_be_updated(existing_user, attributes, user_dn):
    return existing_user.position.getDn().endswith(user_dn)

def _group_should_be_updated(existing_group, attributes, group_dn):
    return existing_group.position.getDn().endswith(group_dn)

def _import_user(user_dn, attributes):
    existing_user = _user_exists(attributes)
    user_dn = getPosition(user_dn)
    if existing_user is None:
        _create_user(user_dn, attributes)
        existing_user = _user_exists(attributes)
    if _user_should_be_updated(existing_user, attributes, user_dn):
...
def _import_group(group_dn, attributes):
    ...
    group_dn = getPosition(group_dn)
    if existing_group is None:
        _create_group(attributes)
        existing_group = _group_exists(attributes)
    if _group_should_be_updated(existing_group, attributes, group_dn):
